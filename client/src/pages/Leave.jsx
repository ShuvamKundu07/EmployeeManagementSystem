...
const Leave = () => {
  ...
  const fetchLeaves = useCallback(async () => {
    try {
      ...
      toast.error(error?.response?.data?.error || error.message || "Failed to fetch leaves")
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  const approveLeaves = leaves.filter((l) => l.status === "APPROVED")
  const sickCount = approveLeaves.filter((l) => l.type === "SICK").length
  const casualCount = approveLeaves.filter((l) => l.type === "CASUAL").length
  const annualCount = approveLeaves.filter((l) => l.type === "ANNUAL").length
          ...
          {leaveStats.map((s) => (
            <div key={s.label} className="card card-hover p-5 sm:p-6 flex items-center gap-4 relative overflow-hidden group">
              ...
              <div>
                ...
                {/* Fixed text-slate-900 trailing period */}
